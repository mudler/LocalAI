...
func (oais *OpenAIService) GenerateFromMultipleMessagesChatRequest(request *schema.OpenAIRequest, endpointConfigFn endpointGenerationConfigurationFn, notifyOnPromptResult bool, notifyOnToken bool, initialTraceID *OpenAIRequestTraceID) (
	...
	go func() {
		...
		for _, result := range rawResult.Value.Response {
			...
			if !processFunctions {
				choiceRes := schema.Choice{Delta: &schema.Message{Content: &result}, Index: 0}
				log.Warn().Msgf("[OAIS GenerateFromMultipleMessagesChatRequest] choiceRes: %+v", choiceRes)

				resp := schema.OpenAIResponse{
					ID:      traceID.ID,
					Created: traceID.Created,
					Model:   request.Model, // we have to return what the user sent here, due to OpenAI spec.
					Choices: []schema.Choice{choiceRes},

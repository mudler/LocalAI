...
# - "6" (default): use packages already present in the rocm/dev-ubuntu-* base image
...
RUN if [ "${BUILD_TYPE}" = "hipblas" ] && [ "${SKIP_DRIVERS}" = "false" ]; then \
    if [ "${ROCM_VERSION}" = "7" ]; then \
        ...
        # amdrocm-llvm: ROCm LLVM/Clang toolchain (compiler for HIP kernels)
        ...
        # Create /opt/rocm/* compat symlinks so cmake find_package(hip) and existing
        # linker flags work unchanged.  Using core-7 (not core-7.XX) means these
        # symlinks survive minor version bumps (7.11 → 7.12 etc.) automatically.
        ...
        ldconfig && \
        # rocWMMA (rocwmma-dev) is not available in the ROCm 7.x apt repo.
        # Install headers from source so GGML_HIP_ROCWMMA_FATTN can be compiled.
        # Headers go to /opt/rocwmma-headers; CPATH is set below so the compiler
        # finds them without any extra cmake path gymnastics.
        ...
        rm -rf /tmp/rocwmma && \
        # rocwmma-version.hpp is generated by cmake configure_file in the source tree.
        # Write it directly to avoid pulling in cmake at this stage.
        printf '#ifndef ROCWMMA_API_VERSION_HPP\n#define ROCWMMA_API_VERSION_HPP\n#define ROCWMMA_VERSION_MAJOR 2\n#define ROCWMMA_VERSION_MINOR 2\n#define ROCWMMA_VERSION_PATCH 0\n#endif\n' \
            > /opt/rocwmma-headers/rocwmma/rocwmma-version.hpp ; \
    else \
...
# Set CPATH so the compiler finds rocWMMA headers during backend compilation.
# This ENV is a no-op on non-hipblas builds (the path does not exist there).
...
# ROCm 7.x: libamd_comgr.so.3 depends on LLVM shared libs (libLLVM.so, libclang-cpp.so)
# that live in /opt/rocm/llvm/lib.  ldconfig alone is insufficient because backend
# subprocesses load libamd_comgr via dlopen() which respects LD_LIBRARY_PATH but not
# ldconfig when the caller bypasses the standard linker.  This path is a no-op on
# non-ROCm builds (the directory simply does not exist).

...
func Run(ctx *cliContext.Context, cfg *Config) error {
	...
	// The readiness gate is created here but only armed once the tunnel and the
	// backend supervisor exist, below, because the gate probes both.
	...
	// The tunnel is the control path, while the supervisor covers the local data
	// path. A worker is ready only when the frontend can reach it and each
	// running backend can still be dialled locally.

...
| `--artifact-download-concurrency` | `1` | How many files of a Hugging Face model artifact to download at once. `1` downloads sequentially. Raising it helps repositories split into many shards on a fast link, at the cost of more concurrent load on the models volume. Whole files only — a single file is never split, so resume and per-file checksum verification are unaffected | `$LOCALAI_ARTIFACT_DOWNLOAD_CONCURRENCY` |

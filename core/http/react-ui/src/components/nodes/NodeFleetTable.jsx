...
function MetricCell({ total, available, suffix = '' }) {
  ...
  return <span>{formatBytes(used)} / {formatBytes(total)}{suffix}</span>
}
        ...
        <tbody>
          {groups.flatMap(group => {
            ...
            const rows = group.nodes.map(node => (
              <tr key={node.id} className="fleet-table__row" tabIndex="0" onClick={() => onInspect(node)}
                ...
                <td><MetricCell total={node.total_vram} available={node.available_vram} /></td>
                <td><MetricCell total={node.total_ram} available={node.available_ram} /></td>

...
func (a *RemoteUnloaderAdapter) stopBackend(ctx context.Context, nodeID, backend string, force bool) error {
	...
	if reply.ReportsStoppedProcesses && len(reply.StoppedProcessKeys) == 0 {
		xlog.Warn("backend.stop matched no running process on the worker", "nodeID", nodeID, "backend", backend)
		return nil
	}
	xlog.Info("Worker stopped backend processes", "nodeID", nodeID, "backend", backend, "stopped", reply.StoppedProcessKeys)
	return nil
}

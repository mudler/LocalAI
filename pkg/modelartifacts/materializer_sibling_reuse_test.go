...
// TestReuseFromCommittedSibling_NarrowAllowPatterns proves the fix for #11047:
...
// os.SameFile true).
func TestReuseFromCommittedSibling_NarrowAllowPatterns(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(broad.CacheHit).To(BeFalse())
	g.Expect(resolver.fetchCount("a/first.bin")).To(BeNumerically(">", 0),
	...
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(narrow.CacheHit).To(BeFalse())
	...
	g.Expect(resolver.fetchCount("a/first.bin")).To(Equal(0),
	...
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(os.SameFile(broadInfo, narrowInfo)).To(BeTrue(),
	...
	g.Expect(os.ReadFile(narrowFile)).To(Equal(contents["a/first.bin"]))
}

// TestBroaderRequestDoesNotServeNarrowSiblingAsComplete is the manifest file-set
// guard the reviewer demanded: a broader request against a narrower committed
// sibling must still fetch the files the sibling lacks and commit a complete
// tree. Sibling-reuse can never serve an incomplete model as complete, because
// each requested file is matched individually against the sibling's manifest.
func TestBroaderRequestDoesNotServeNarrowSiblingAsComplete(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(narrowPaths).To(Equal([]string{"a/first.bin", "b/second.bin"}))
	...
	g.Expect(resolver.fetchCount("c/third.bin")).To(BeNumerically(">", 0),
	...
	g.Expect(broadPaths).To(Equal([]string{"a/first.bin", "b/second.bin", "c/third.bin"}))

	// Every file is present on disk with the right bytes after commit.
	for _, p := range []string{"a/first.bin", "b/second.bin", "c/third.bin"} {
		g.Expect(os.ReadFile(filepath.Join(modelsPath, filepath.FromSlash(broad.RelativePath), filepath.FromSlash(p)))).
			To(Equal(contents[p]))
	}
}

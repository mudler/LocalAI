...
// TogglePinned pins or unpins a model. action must be "pin" or "unpin".
// syncPinned, if non-nil, is invoked after a successful reload so the
// watchdog can refresh its eviction-exempt set.
func (s *ConfigService) TogglePinned(_ context.Context, name, action string, syncPinned SyncPinnedFn) (*ToggleResult, error) {
	if name == "" {
		return nil, ErrNameRequired
	}
	if action != "pin" && action != "unpin" {
		return nil, fmt.Errorf("%w: must be 'pin' or 'unpin', got %q", ErrBadAction, action)
	}
	...
	if err := mutateYAMLBoolFlag(configPath, "pinned", action == "pin"); err != nil {
		return nil, err
	}

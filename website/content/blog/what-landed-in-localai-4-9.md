...
summary: "Authentication now denies by default, chat can compress its own history, and models and backends each got one page instead of three. 146 pull requests in thirteen days."
extracss: ["blog.css"]
---
...
A long conversation eventually stops fitting in the context window, and the usual answer is to drop the oldest messages. This release adds a different one: compress them.
...
Several things are deliberately left alone. Leading system and developer prompts are preserved, so a safety prompt is not summarized away. The newest messages are preserved. Tool-call and tool-result pairs are kept whole, so a compressed history never contains a call whose result went missing. Compression runs after PII filtering and after Assistant and MCP prompt injection, including on later MCP iterations, so what gets compressed is the prompt that would actually have been sent rather than an earlier version of it. Tool schemas and the headroom needed for the completion are accounted for with a conservative offline token bound.
...
The rebuild moves to a wider layout with the format reference beside the work column instead of behind a chevron that was closed by default. That reference answers the only question a first-time admin has, which is what you are allowed to paste. Below 1024px it becomes a disclosure rather than vanishing. The source field is the hero and carries its own Import button, which removed the `aria-hidden` submit button that only existed because the real action sat outside the `<form>`. Simple and Advanced modes are gone: they were about 80% the same surface, and keeping both cost a mode switch, a localStorage key and a Keep/Discard/Cancel dialog whose only job was protecting state the switch would have hidden. What actually differs is the kind of input, so that is now two tabs, a source or YAML ([#11461](https://github.com/mudler/LocalAI/pull/11461)).
...
The split follows where the silicon exists. Jetson is arm64 only, `87` for Orin and `110` for Thor. Desktop `120a` is amd64 only. `90a` and `100a` are on both because of the SBSA parts. That adds A100, A10 and 3090, L4, 4090 and RTX 6000 Ada, H100 and H200, B200, B300, Jetson Orin and Jetson Thor. The CUDA 13 guard now covers both branches instead of amd64 alone, and Triton-AOT stays on ([#11512](https://github.com/mudler/LocalAI/pull/11512)).

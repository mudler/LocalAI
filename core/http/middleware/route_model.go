...
// unless classifier_model's config declares FLAG_SCORE. Scoring runs on
// the backend's server task queue, so it is safe to combine with other
// usecases — this check only refuses to bind a model that never declared
// itself for Score in the first place; that model could be a misconfigured
// chat model the operator pointed at by accident.
//

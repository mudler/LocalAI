...
func ReadApiKeysJson(configDir string, options *options.Option) error {
...
func ReadExternalBackendsJson(configDir string, options *options.Option) error {
...
var CONFIG_FILE_UPDATES = map[string]func(configDir string, options *options.Option) error{
	"api_keys.json":          ReadApiKeysJson,
	"external_backends.json": ReadExternalBackendsJson,
}

func WatchConfigDirectory(configDir string, options *options.Option) (WatchConfigDirectoryCloser, error) {
	if len(configDir) == 0 {

...
func ChatEndpoint(cl *config.ConfigLoader, ml *model.ModelLoader, startupOptions *schema.StartupOptions) func(c *fiber.Ctx) error {
	emptyMessage := ""
	id := uuid.New().String()
...
func handleQuestion(config *config.Config, input *schema.OpenAIRequest, ml *model.ModelLoader, o *schema.StartupOptions, args, prompt string) (string, error) {

...
#include "common.h"
// chat-auto-parser.h is not available in ik_llama.cpp; tool_format detection
// in ModelMetadata is skipped — the Go side uses its own fallback parsing.
#include <getopt.h>
    ...
    grpc::Status PredictStream(grpc::ServerContext* context, const backend::PredictOptions* request, grpc::ServerWriter<backend::Reply>* writer) override {
        ...
        try {
            ...
            for (size_t i = 0; i < inputs.size(); i++) {
                ...
                // OAI-compat: enable chat parsing so that reasoning, tool calls,
                // and content are classified into ChatDeltas.
...
// request slots data using task queue
    ...
    grpc::Status ModelMetadata(ServerContext* /*context*/, const backend::ModelOptions* /*request*/, backend::ModelMetadataResponse* response) override {
        ...
        // NOTE: ik_llama.cpp does not provide chat-auto-parser.h (the autoparser
        // namespace used for differential tool-format detection from chat templates).
        // tool_format is left unpopulated; the Go side uses its own fallback
        // parsing path for tool calls, which works correctly without this metadata.

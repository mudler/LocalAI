...
if [ "x${BUILD_TYPE}" == "xhipblas" ]; then
    ...
    # GPU arch(es) to build for. AMDGPU_TARGETS is the repo-wide comma-separated
    # gfx list (see backend/cpp/llama-cpp/Makefile); local `make` builds outside CI
    # pass none, and gfx1151 is the arch this was hardware-validated on.
    _gpu_targets="${AMDGPU_TARGETS:-gfx1151}"

    # AMD publishes a per-GPU torch on its multi-arch index, selected with the
    # torch[device-gfx<arch>] extra. The extras compose, so a multi-arch target
    # list installs one device package per arch -- each ~1.6 GB installed.
    _gpu_extras=""
    for _a in ${_gpu_targets//,/ }; do
        _a="${_a%% *}"
        [ -n "${_a}" ] || continue
        _gpu_extras="${_gpu_extras:+${_gpu_extras},}device-${_a}"
    done

    # Does the prebuilt ROCm wheel cover EVERY requested arch? It is CDNA-only.
    # One uncovered arch is enough to need the source build, because a single venv
    # holds a single vllm.
    _vllm_prebuilt=true
    for _a in ${_gpu_targets//,/ }; do
        case "${_a%% *}" in
            ""|gfx942|gfx950) ;;
            *) _vllm_prebuilt=false ;;
        esac
    done
    echo "${0##*/}: AMD targets ${_gpu_targets} -> prebuilt vllm wheel: ${_vllm_prebuilt}"
fi
...
# https://github.com/vllm-project/vllm/blob/main/docs/getting_started/installation/gpu.xpu.inc.md
    ...
    popd
# AMD ROCm. Two paths, picked by the target arch, because the prebuilt wheel exists
# for only one of them:
#
#   * CDNA data-center (gfx942 / gfx950) -> the prebuilt wheel from
#     wheels.vllm.ai/rocm, exactly as before. Unchanged, and still what the
#     published ROCm image builds.
#   * anything else, e.g. gfx1151 (RDNA 3.5, Strix Halo) -> build from source.
#     There is NO prebuilt vllm wheel for these GPUs anywhere: the wheels.vllm.ai
#     build is CDNA-only and pinned to a rocm7.2.3 torch that cannot even enumerate
#     Strix Halo (device_count == 0), and AMD's multi-arch index carries torch and
#     triton but no vllm. That is why AMD ships a separate rocm/vllm:*_rdna_*
#     container; the branch below reproduces its recipe inside LocalAI's normal
#     Dockerfile.python build (torch 2.11.0+rocm7.14.0, vllm 0.23.1.dev1 @ 9ddef7117),
#     mirroring the intel/cpu source-build branches above.
#
# Deciding on the arch instead of replacing the prebuilt path keeps the published
# CDNA image what it is today, build time included. _gpu_targets / _vllm_prebuilt
# are computed in the pre-venv hipblas block above.
elif [ "x${BUILD_TYPE}" == "xhipblas" ] && [ "${_vllm_prebuilt}" = "true" ]; then
    installRequirements
    ...
    uv pip install --index-url https://repo.amd.com/rocm/whl-multi-arch/ \
        "torch[${_gpu_extras}]==2.11.0+rocm7.14.0" \

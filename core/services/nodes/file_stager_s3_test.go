...
var _ = Describe("the S3 file stager's control RPCs", func() {
	...
	DescribeTable("addresses the verb that verb's path names",
		...
		Entry("mkdir", workerctl.PathFilesMkdir, fileMkdirReply{LocalPath: "/w/models/export"},
			func(s *S3FileStager) error {
	...
	It("refuses cleanup that crosses its declared storage namespace", func() {
		for _, key := range []string{"models/../data/victim", "data/../models/victim"} {
			Expect(stager.ReleaseRemoteDir(context.Background(), nodeID, key)).To(MatchError(ContainSubstring("invalid staging directory")))
		}
		Expect(workers.callSubjects()).To(BeEmpty())
	})
	...
	DescribeTable("reports the worker's own refusal as the worker's answer",
		...
		Entry("mkdir", workerctl.PathFilesMkdir, func(s *S3FileStager) error {
			_, err := s.AllocRemoteDir(context.Background(), nodeID, "models/export")
			return err
		}),
